...
This is a test version of the stock portfolio summary. 

#Note:
- change wd to more appropiate
- might need to use reshape2
- Add geography category (e.g. Continents or countries maybe)
- Need account for the average purchase and selling prices. At the moment, we are calculating the overall portfolio value based on the closing value of a stock.
- asked Barry what he thinks about changing point (pence) to pounds
- need to build a summary for the pie-chart based on Portfolio on the last weekday before Sys.Date 
...
# this is empty
dir()
df_PF_input <- vroom("ETF_Share.csv",col_types = c(EPIC = "f", Name = "f", Buy_Sell = "f", Type = "f", Sector = "f", Market = "f")) # "f" for factor, 
...
#df_temp <- aggregate(df_PF_input["cs_All_Inv"],by = df_PF_input["Date"], sum )
#df_temp$cs_All_Inv <- cumsum(df_temp$cs_All_Inv)
#df_PF_input$cs_All_Inv <- left_join(df_PF_input, df_temp, by = "Date")$cs_All_Inv.y
#rm(df_temp)
...
#df_temp <- aggregate(df_PF_input["cs_ETF_Share_Inv"],by = df_PF_input[c("Date", "Type")], sum )
#df_temp <- df_temp %>% group_by(Type) %>% mutate(cs_ETF_Share_Inv = cumsum(cs_ETF_Share_Inv)) %>% ungroup()
#df_PF_input$cs_ETF_Share_Inv <- left_join(df_PF_input, df_temp, by = c("Date","Type"))$cs_ETF_Share_Inv.y
#rm(df_temp)
...
API_KEY_2 <- "E0FOXEVH0PJN42RA"
API_KEY_3 <- "ZSDYFZL73TSULLRI"
API_KEY_4 <- "66DBPI7BYN4OFIEC"
API_KEY_5 <- "ZLAULT9DJDDNRRSH"
...
# reshape the data.frame to have all stock values in column
...
subset(df_ETF_Share_ts, Date %in% max(df_ETF_Share_ts$Date))%>%
  ...
  PieDonut(., aes(Market, Sector, count=PF_Stock_Value_sum),
         title = "Overall Market & Sector distribution",
         color = "black",
                  ratioByGroup = FALSE)
...
subset(df_ETF_Share_ts, Date %in% max(df_ETF_Share_ts$Date))%>%
  ...
  PieDonut(., aes(Sector, EPIC, count=PF_Stock_Value),
         title = "Overall Sector & Stock distribution",
         color = "black",
                  ratioByGroup = FALSE)
...
```{r  Overall Portfolio preformance}
...
```{r Portfolio relative gain and losses}
