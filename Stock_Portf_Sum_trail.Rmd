...
df_PF_div <- vroom("dividend.csv",col_types = c(EPIC = "f", Name = "f", Type = "f")) # "f" for factor, 
...
```{r Current distributions, echo=FALSE}
...
```{r Overall Portfolio preformance, echo=FALSE}
...
df_ETF_Share_ts %>% 
  select(Date,cs_All_Inv,cs_All_Value) %>% 
  distinct() %>% 
...
```{r Portfolio relative gain and losses, echo=FALSE}
...
```{r Share preformance, echo=FALSE}
...
```{r Calculating the dividend per stock, include=FALSE}
...
ggplot(., aes(x = Year ,y = Sum, fill=EPIC ))+
  geom_col(stat = "identity", colour = "Black", position = position_dodge(), alpha=0.7)+
