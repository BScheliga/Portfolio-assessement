...
df_PF_input %>% filter(EPIC %in%  df_Stocks_sold$EPIC) %>% select(EPIC,Buy_Sell, Aver_cost, Volume, Date, Trad_cost) %>% group_by(EPIC) %>%   summarise(Gain_Losses_GPB = -sum(Aver_cost*Volume)/100,Overall_Trad_cost = sum(Trad_cost), Time_held = max(Date)-min(Date)) %>% 
  ...
  summarise(EPIC=EPIC, "Bought [£]" = round(Bought_For,2), "Sold [£]" = round(Sold_For,2), "Gain & Losses [£]" = round(Gain_Losses_GPB,2), "Gain & Losses [%]" = round(Gain_Losses_GPB/Sold_For*100,1),"Gain & Losses corrected* [£]" = round((Gain_Losses_GPB-Overall_Trad_cost+na.omit(Over_Div)),2), "Time held" = Time_held) %>% kable()
