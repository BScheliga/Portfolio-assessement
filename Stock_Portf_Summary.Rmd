...
# Sells (*Find better title later* and move this calucaltion update)

|EPIC|Bought for|Sold for| Gains & Losses £|Gains & Losses %|Gains & Losses incl. Trading costs & Dividends £|Bought when| Sold when| Hold for|

```{r Stats on stock sells, echo=FALSE}
# I only need to rinput table & the dividend table
...
df_PF_input %>% filter(EPIC == df_Stocks_sold$EPIC) %>% select(EPIC,,Buy_Sell, Aver_cost, Volume, Date, Trad_cost)



df_PF_input %>% filter(EPIC == df_Stocks_sold$EPIC) %>% group_by(EPIC) %>% summarise(n = sum(Aver_cost*Volume))
  
  filter(Buy_Sell == c("SELL", "BUY"))
