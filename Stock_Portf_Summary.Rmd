...
df_PF_input %>% filter(EPIC == df_Stocks_sold$EPIC) %>% select(EPIC,Buy_Sell, Aver_cost, Volume, Date, Trad_cost) %>% group_by(EPIC) %>%   summarise(Gain_Losses_GPB = -sum(Aver_cost*Volume)/100,Overall_Trad_cost = sum(Trad_cost), Time_held = max(Date)-min(Date)) %>% 
  # calculating overall buying cost 
  left_join(df_PF_input %>% filter(EPIC == df_Stocks_sold$EPIC & Buy_Sell == "BUY" ) %>% select(EPIC,Buy_Sell, Aver_cost, Volume) %>%  group_by(EPIC) %>% summarise(Bought_For = sum(Aver_cost*Volume)/100),by = "EPIC") %>% 
  # calculating overall selling price
   left_join(df_PF_input %>% filter(EPIC == df_Stocks_sold$EPIC & Buy_Sell == "SELL" ) %>% select(EPIC,Buy_Sell, Aver_cost, Volume) %>%  group_by(EPIC) %>% summarise(Sold_For = sum(Aver_cost*abs(Volume))/100),by = "EPIC") %>% 
  # adding Dividends payouts
  left_join(df_PF_div %>% 
              filter(EPIC == df_Stocks_sold$EPIC) %>% group_by(EPIC) %>%               summarise(Over_Div = sum(Amount_Payable)),by = "EPIC") %>% # setting the order of the table
  relocate(c(Bought_For,Sold_For), .after=EPIC) %>% relocate(Over_Div, .after= Overall_Trad_cost) %>% 
  # Final sumarise, I promise
  summarise(EPIC=EPIC, "Bought [£]" = round(Bought_For,2), "Sold [£]" = round(Sold_For,2), "Gain & Losses [£]" = round(Gain_Losses_GPB,2), "Gain & Losses [%]" = round(Gain_Losses_GPB/Sold_For*100,1),"Gain & Losses corrected* [£]" = (Gain_Losses_GPB-Overall_Trad_cost+Over_Div), "Time held" = Time_held) %>% kable()
